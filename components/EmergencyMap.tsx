...
const EmergencyMap: React.FC<EmergencyMapProps> = ({ hospitals, selectedHospitalId, onSelectHospital, userLocation }) => {
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Ensure map fills container if initialized before full width is available
    setTimeout(() => {
  ...
  useEffect(() => {
    if (!mapRef.current || !userLocation || hospitals.length === 0) return;

    // FIX: latLngBounds expects a LatLngBoundsExpression, which can be LatLngExpression[].
    // Wrapping the coordinates in another array ensures it's interpreted as an array of points.
  ...
  useEffect(() => {
    ...
    if (hospital) {
      mapRef.current.panTo([hospital.lat, hospital.lng], { animate: true });
  ...
  useEffect(() => {
    ...
    hospitals.forEach(hospital => {
      ...
      const icon = L.divIcon({
        ...
        html: `
          <div class="flex flex-col items-center transition-all duration-300 ${isSelected ? 'scale-125 z-[1000]' : 'scale-100'}">
      ...
      if (markersRef.current[hospital.id]) {
        ...
        markersRef.current[hospital.id].setZIndexOffset(isSelected ? 1000 : 0);
      } else {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-slate-100 rounded-3xl border-4 border-white shadow-2xl aspect-[16/9] lg:aspect-[3/2]">
      ...
      {/* Legend overlay */}
      <div className="absolute bottom-6 left-6 z-[1000] bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-white/50 flex flex-col gap-3">
